...
class TradingStrategy:
    def __init__(self, symbol):
        ...
        self.trend_data = CandlesData(self.symbol).get_trend_data()
        self.pattern_data = CandlesData(self.symbol).get_pattern_indicators_data()
    ...
    def find_trend(self, htf_df):
        ...
        last = {
            ...
            'kijun': self.trend_indicators['kijun_sen'].iloc[-1],  # https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D0%B4%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80_%D0%98%D1%88%D0%B8%D0%BC%D0%BE%D0%BA%D1%83
            'senkou_a': self.trend_indicators['senkou_span_a'].iloc[-1], # https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D0%B4%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80_%D0%98%D1%88%D0%B8%D0%BC%D0%BE%D0%BA%D1%83
            'senkou_b': self.trend_indicators['senkou_span_b'].iloc[-1], # https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D0%B4%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80_%D0%98%D1%88%D0%B8%D0%BC%D0%BE%D0%BA%D1%83
            # индикаторы на 50 свеч макс по младшему тф
            'ema50': self.pattern_indicators['ema50'].iloc[-1], # экспоненциальная скользящая средняя
            'rsi12': self.pattern_indicators['rsi12'].iloc[-1], # индекс перекупленности и перепроаднности 12 свеч младший таймфрейм
            'rsi25': self.pattern_indicators['rsi25'].iloc[-1], # индекс перекупленности и перепроаднности 25 свеч младший фрейм
            'atr7': self.pattern_indicators['atr7'].iloc[-1],  # волатильность рынка поддержки
            'macd': self.pattern_indicators['macd'].iloc[-1], # https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D0%B4%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80_MACD
            'macd_signal': self.pattern_indicators['macd_signal'].iloc[-1] # https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D0%B4%D0%B8%D0%BA%D0%B0%D1%82%D0%BE%D1%80_MACD
        }
    ...
    def return_signal(self):
        self.trend_direction, self.trend_strength, self.last = self.find_trend(self.trend_data)
        ...
        self.sl, self.tp = self.sl_tp(self.last, self.signal)
